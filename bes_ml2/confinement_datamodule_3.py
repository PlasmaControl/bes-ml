...
# from bes_data.confinement_data_tools import bad_confinement_indices_csv
...
class Confinement_TrainValTest_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # assert torch.max(self.sample_indices)+self.signal_window_size-1 < self.labels.shape[0]
    ...
    #     # confinement_mode_id_int = int(confinement_mode_key.replace('_', ''))
...
class Confinement_Predict_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i: int) -> tuple:
        ...
        label = self.labels[ label_index : label_index + 1 ]
        # label_class = torch.tensor([0]) if label >= 0 else torch.tensor([1])
        print(f"Type of signal_window: {type(signal_window)}, shape: {signal_window.shape}")
        print(f"Type of label: {type(label)}, shape: {label.shape}")
        print(f"Type of shot: {type(self.shot)}, shot: {self.shot}")
        print(f"Type of start_time: {type(self.start_time)}, start_time: {self.start_time}")
...
class Confinement_Datamodule(LightningDataModule):
    ...
    r_avg_bounds: tuple = None
    z_avg_bounds: tuple = None
    delz_avg_bounds: tuple = None
    r_avg_bounds_class_3: tuple = None
    z_avg_bounds_class_3: tuple = None
    delz_avg_bounds_class_3: tuple = None
    max_shots: int = None
    ...
    def __post_init__(self):
        ...
        # self._get_confinement_indices_and_split()
    ...
    def setup(self, stage=None):
        ...
        for dataset_stage in dataset_stages:
            ...
            # times = [self.get_time_for_index(index) for index in indices]
            times = [self.get_time_for_index_2(shot_event) for shot_event in events]  # Adapted for (shot, event) tuples
            ...
            # dataset = self._load_and_preprocess_data(chunk_indices, dataset_stage)
    ...
    def get_time_for_index(self, confinement_mode_index):
        with h5py.File(self.data_file, 'r') as h5_file:
            confinement_mode_key = f"{confinement_mode_index}"
            time_count = h5_file[confinement_mode_key]["signals"].shape[1]
        return time_count
    
    def get_time_for_index_2(self, shot_event_tuple):
    ...
    def _load_and_preprocess_data(self, indices, dataset_stage):            
        t0 = time.time()
        # package confinement events into pytorch dataset
        ...
        with h5py.File(self.data_file, 'r') as h5_file:
            ...
            time_counts = []
            long_enough_indices = []  # List to hold indices of confinement modes with long enough signals
            for i_confinement_mode, confinement_mode_index in enumerate(indices):
                confinement_mode_key = f"{confinement_mode_index}"
                signal_length = h5_file[confinement_mode_key]["signals"].shape[1]
                ...
                if signal_length >= self.signal_window_size:
                    ...
                    long_enough_indices.append(confinement_mode_index)

            time_count = np.sum(time_counts)
            discarded_count = len(indices) - len(long_enough_indices)
            print(f"Discarded {discarded_count} indices due to insufficient signal length.")
            packaged_signals = np.empty((time_count, self.n_rows, self.n_cols), dtype=np.float32)
            start_index = 0
            for i_confinement_mode, confinement_mode_index in enumerate(long_enough_indices):
                if i_confinement_mode%100 == 0:
                    print(f"  Reading confinement mode event {i_confinement_mode:04d}/{len(indices):04d}")
                confinement_mode_key = f"{confinement_mode_index}"
                confinement_mode_event = h5_file[confinement_mode_key]
                signals = np.array(confinement_mode_event["signals"][:selected_channels, :], dtype=np.float32)  # (selected, <time>)
                signals = np.transpose(signals, (1, 0)).reshape(-1, self.n_rows, self.n_cols)  # reshape to (<time>, self.n_rows, self.n_cols)
                signals = self.apply_bandpass_filter(signals)
                labels = np.array(confinement_mode_event["labels"], dtype=int)
                ...
                confinement_data.append({
                    'labels': labels, 
                    'valid_t0': valid_t0,
                    'confinement_mode_key': confinement_mode_key,
                    'shot': confinement_mode_key.split('_')[0],
                    # 'time': confinement_mode_key.split('_')[0][6:],
                    'time': confinement_mode_key.split('_')[1],
        ...
        if self.one_hot_labels:
            encoder = OneHotEncoder(sparse_output=False, categories=[np.arange(self.num_classes)])
            # Reshape labels to a 2D array and perform one-hot encoding
            packaged_labels = encoder.fit_transform(packaged_labels.reshape(-1, 1))
            print(packaged_labels.shape)
        ...
        if self.mask_sigma_outliers:
            if None in [self.mask_lb, self.mask_ub]:
                assert dataset_stage == 'train' or not self.train_confinement_indices, f"Dataset_stage: {dataset_stage}"
    ...
    def _load_and_preprocess_data_2(self, shot_event_indices, dataset_stage):
        ...
        with h5py.File(self.data_file, 'r') as h5_file:
            ...
            for i, (shot, event) in enumerate(long_enough_indices):
                ...
                # signals = np.array(event_data["signals"][:selected_channels, :], dtype=np.float32)
    ...
    def reshape_signals_6x8(self, signals, inboard_order):
        ...
        # Debug output to understand the incoming shapes and orders
        # print(f"Using inboard_order: {inboard_order}")
        ...
        with h5py.File(self.data_file, "r") as data_file:
            ...
            for shot in data_file.keys():
                if shot in self.bad_shots:
                    ...
                    continue  # Skip processing of the shot if it's in the bad shots list
                shot_metadata = data_file[shot].attrs
                r_avg = shot_metadata.get('r_avg')
                z_avg = shot_metadata.get('z_avg')
                delz_avg = shot_metadata.get('delz_avg')

                # Find the mode (most common value) of the labels array for this shot
                shot_labels = []
                ...
                # Skip the shot if no labels are found
                if not shot_labels:
                    continue
                
                most_common_label = mode(shot_labels).mode

                # Apply different bounds for class 3
                if most_common_label == 3:
                    r_avg_bounds = self.r_avg_bounds_class_3 if self.r_avg_bounds_class_3 is not None else self.r_avg_bounds
                    z_avg_bounds = self.z_avg_bounds_class_3 if self.z_avg_bounds_class_3 is not None else self.z_avg_bounds
                    delz_avg_bounds = self.delz_avg_bounds_class_3 if self.delz_avg_bounds_class_3 is not None else self.delz_avg_bounds
                else:
                    r_avg_bounds = self.r_avg_bounds
                    z_avg_bounds = self.z_avg_bounds
                    delz_avg_bounds = self.delz_avg_bounds

                # Check if metadata falls within the specified bounds
                if r_avg_bounds and (r_avg is None or not (r_avg_bounds[0] <= r_avg <= r_avg_bounds[1])):
                    r_avg_exclusions += 1
                    continue
                if z_avg_bounds and (z_avg is None or not (z_avg_bounds[0] <= z_avg <= z_avg_bounds[1])):
                    z_avg_exclusions += 1
                    continue
                if delz_avg_bounds and (delz_avg is None or not (delz_avg_bounds[0] <= delz_avg <= delz_avg_bounds[1])):
                    delz_avg_exclusions += 1
                    continue
                
                # Store valid events and their labels
                shot_events = [(shot, event) for event in data_file[shot].keys() if 'labels' in data_file[shot][event]]
                shots[shot] = (shot_events, most_common_label)

            # Step 1: Remove the specific shot numbers if they exist in the dataset
            test_shot_data = {}
            if self.force_test_shots:
                for shot_number in self.force_test_shots:
                    if shot_number in shots:
                        ...
                        print(f"Warning: Shot number {shot_number} not found in dataset")
            ...
            # Now, limit the number of shots per class if needed
            shots_by_class = {}
            for shot in shots.keys():
                label = shots[shot][1]  # Assuming [1] is where the label is stored
                if label not in shots_by_class:
                    shots_by_class[label] = []
                shots_by_class[label].append(shot)

            if self.max_shots_per_class is not None:
                for label, shot_list in shots_by_class.items():
                    if len(shot_list) > self.max_shots_per_class:
                        shots_by_class[label] = np.random.choice(shot_list, self.max_shots_per_class, replace=False).tolist()

            if self.max_shots_per_class:
                # Directly create filtered_shots from shots_by_class, as it now accurately represents the desired dataset
                filtered_shots = {shot: shots[shot] for label_shots in shots_by_class.values() for shot in label_shots}
            else:
                # If no class-based limitation is applied, simply use the original shots dictionary
                filtered_shots = shots  
                              
            # Create train, validation and test datasets
            shot_numbers = np.array(list(filtered_shots.keys()))
            mode_times_seconds, mode_shot_counts = self.calculate_mode_times_and_shots(self.data_file, list(filtered_shots.keys()))
            ...
            # Print the number of exclusions for each type of restriction
            ...
            # shuffle confinement mode indices
            print(f"  Shuffling confinement mode events with RNG seed {self.seed}")
            np.random.default_rng(seed=self.seed).shuffle(shot_numbers)

            # Limit the number of shots to self.max_shots if it is defined
            if self.max_shots:
                shot_numbers = shot_numbers[:self.max_shots]
                print(f"  Limiting data to {self.max_shots} unique shots")
            self.all_confinement_events = np.concatenate([filtered_shots[shot_number][0] for shot_number in shot_numbers])

            if not self.test_only:
                labels = [filtered_shots[shot_number][1] for shot_number in shot_numbers]

                # Step 2: Perform the train-test-validation split
                if self.fraction_test == 0 and self.fraction_validation == 0:
                    shot_numbers_train = shot_numbers
                    shot_numbers_val = np.array([])
                    shot_numbers_test = np.array([])
                else:
                    shot_numbers_train, shot_numbers_test_val, _, _ = train_test_split(shot_numbers, labels, test_size=self.fraction_test + self.fraction_validation, stratify=labels, random_state=self.seed)
                    shot_numbers_test, shot_numbers_val, _, _ = train_test_split(shot_numbers_test_val, [filtered_shots[shot_number][1] for shot_number in shot_numbers_test_val], test_size=self.fraction_test/(self.fraction_test + self.fraction_validation), stratify=[filtered_shots[shot_number][1] for shot_number in shot_numbers_test_val], random_state=self.seed)
                    
                # Step 3: Add the forced test shot numbers back to the test set and likewise for validation set
                for shot_number, data in test_shot_data.items():
                    shot_numbers_test = np.append(shot_numbers_test, shot_number)
                    filtered_shots[shot_number] = data
                
                for shot_number, data in validation_shot_data.items():
                    shot_numbers_val = np.append(shot_numbers_val, shot_number)
                    filtered_shots[shot_number] = data

                print(f"  Train data shot numbers: {shot_numbers_train}")
                print(f"  Validation data shot numbers: {shot_numbers_val}")
                print(f"  Test data shot numbers: {shot_numbers_test}")
                # print(f"Overlap between  shot numbers: {overlap}")
                
        if not self.test_only:
            # After splitting, modify the concatenation of indices:
            self.train_confinement_events = [event for shot in shot_numbers_train for event in filtered_shots[shot][0]]
            self.validation_confinement_events = [event for shot in shot_numbers_val for event in filtered_shots[shot][0]]
            self.test_confinement_events = [event for shot in shot_numbers_test for event in filtered_shots[shot][0]]

            # Print the number of events in each set
            print(f"Train events: {len(self.train_confinement_events)}")
            print(f"Validation events: {len(self.validation_confinement_events)}")
            print(f"Test events: {len(self.test_confinement_events)}")
        else:
    ...
    def calculate_mode_times_and_shots(self, file_path, valid_shot_keys):
        ...
        return mode_times_seconds, mode_shot_counts


    def _get_confinement_indices_and_split(self):
        if self.all_confinement_indices is not None:
            print("Reusing previous confinement mode indices read and split")
            return
        print(f"  Data file: {self.data_file}")
        # gather confinement mode indices
        with h5py.File(self.data_file, "r") as confinement_mode_h5:
            print(f"  confinement mode events in data file: {len(confinement_mode_h5)}")
            self.all_confinement_indices = [confinement_mode_key for confinement_mode_key in confinement_mode_h5]
        # bad confinement mode events to ignore?
        if self.bad_confinement_indices or self.bad_confinement_indices_csv:
            if self.bad_confinement_indices is None:
                self.bad_confinement_indices = []
            if self.bad_confinement_indices_csv is True:
                pass
                # self.bad_confinement_indices_csv = bad_confinement_indices_csv
            if self.bad_confinement_indices_csv:
                print(f"  Ignoring confinement mode events from {self.bad_confinement_indices_csv}")
                with Path(self.bad_confinement_indices_csv).open() as file:
                    self.bad_confinement_indices = [int(line) for line in file]
            ignored_confinement_count = 0
            for bad_confinement_index in self.bad_confinement_indices:
                if bad_confinement_index in self.all_confinement_indices:
                    self.all_confinement_indices.remove(bad_confinement_index)
                    ignored_confinement_count += 1
            print(f"  Ignored confinement mode events: {ignored_confinement_count}")
            print(f"  Usable confinement events: {len(self.all_confinement_indices)}")
        with h5py.File(self.data_file, "r") as data_file:
            print(f"  Confinement modes in data file: {len(data_file)}")
            ...
            # Create train, validation and test datasets
            shot_numbers = np.array(list(shots.keys()))
            ...
            if self.max_shots:
                shot_numbers = shot_numbers[:self.max_shots]
                print(f"  Limiting data to {self.max_shots} unique shots")
            self.all_confinement_indices = np.concatenate([shots[shot_number][0] for shot_number in shot_numbers])

            labels = [shots[shot_number][1] for shot_number in shot_numbers]
            shot_numbers_train, shot_numbers_test_val, _, _ = train_test_split(shot_numbers, labels, test_size=self.fraction_test + self.fraction_validation, stratify=labels, random_state=self.seed)
            ...
            self.train_confinement_indices = np.concatenate([shots[shot_number][0] for shot_number in shot_numbers_train])
            self.validation_confinement_indices = np.concatenate([shots[shot_number][0] for shot_number in shot_numbers_val])
            self.test_confinement_indices = np.concatenate([shots[shot_number][0] for shot_number in shot_numbers_test])

        print(f"Total confinement mode events  {self.all_confinement_indices.size}")
        print(f"  Train  {self.train_confinement_indices.size}  ({self.train_confinement_indices.size/self.all_confinement_indices.size*100:.1f}%)")
        print(f"  Validation  {self.validation_confinement_indices.size}  ({self.validation_confinement_indices.size/self.all_confinement_indices.size*100:.1f}%)")
        print(f"  Test  {self.test_confinement_indices.size}  ({self.test_confinement_indices.size/self.all_confinement_indices.size*100:.1f}%)")
    ...
    #     # determine valid t0 indices (start of signal windows) for each confinement event
