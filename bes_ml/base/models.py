...
class FFT_Features(_FFT_Features_Dataclass, _Base_Features):
    ...
    def forward(self, x):
        ...
        # for i_subwindow, subwindow in enumerate(x.split(self.subwindow_size, dim=4)):
        #     for i_bin in range(self.fft_nbins):
        #         rfft = torch.fft.rfft(
        #             subwindow[:, :, :, :, i_bin * self.nfft:(i_bin+1) * self.nfft, :, :], 
        #             dim=4,
        #         )
        #         fft_bins[:, i_subwindow:i_subwindow+1, i_bin:i_bin+1, :, :, :, :] = torch.abs(rfft) ** 2
        # fft_subwindows = torch.mean(fft_bins, dim=2, keepdim=False)
        # fft_subwindows[fft_subwindows<1e-5] = 1e-5
        # fft_subwindows = torch.log10(fft_subwindows)
        ...
        #     self.fft_min = np.min([fft_subwindows.min().item(), self.fft_min])
        #     self.fft_max = np.max([fft_subwindows.max().item(), self.fft_min])
        #     hist, bin_edges = np.histogram(
        #         fft_subwindows.cpu(),
        #         bins=self.hist_bins,
        #         range=[-7,7],
