...
class Trainer(_Trainer_Base):
    ...
    def _get_valid_indices(
        ...
        # logging.warning_finish_subclass_initialization(f"valid_t0:{valid_t0}")
        # labels = np.arange(active_elm_start_index, 1, -1, dtype=float) / 10.0**3
        labels = (np.arange(active_elm_start_index, 1, -1, dtype=float) - 500) / 10.0**3
        signals = signals[:active_elm_start_index-1, :, :]
        valid_t0[0:active_elm_start_index-1001] = 0
        # labels = np.arange(2001, 1, -1, dtype=float) / 10^-3
        # signals = signals[active_elm_start_index-2001:active_elm_start_index-1, :, :]
        if self.log_time:
            if np.any(labels == 0):
                assert False
