...
class _Trainer_Base(object):
    ...
    def _load_model_parameters(
        ...
        if self.is_regression:
            ...
            # checkpoint_file = Path('model_class_onehot_v1') / 'checkpoint.pytorch' 
            checkpoint_file = Path('model_class_onehot_v2') / 'checkpoint.pytorch' 
    ...
    def train(self) -> None:
        ...
        for i_epoch in range(self.n_epochs):
            ...
            if self.is_regression:
                ...
                # prediction_labels = torch.round(torch.nn.Softmax(dim=1)(torch.tensor(predictions))).argmax(1).cpu().detach().numpy()[:,np.newaxis]
                # self.logger.info(f"prediction_labels {predictions, predictions.shape}")                
                # self.logger.info(f"true_labels {true_labels, true_labels.shape}")                
    ...
    def _single_epoch_loop(
        ...
        with context:
            for i_batch, (signal_windows, labels) in enumerate(data_loader):
                ...
                # )
                # self.logger.info(f"predictions.squeeze() {predictions.squeeze().dtype, predictions.squeeze().shape} s")
                # self.logger.info(f"labels.type_as(predictions) {labels.dtype, labels.shape} s")
                # self.logger.info(f"labels.type_as(predictions) {labels.type_as(predictions).dtype, labels.type_as(predictions).shape} s")
                loss = self.loss_function(
                    predictions.squeeze(),
                    labels,
                )
                ...
                #     apply sigmoid to get [0,1] probability
                #     predictions = predictions.sigmoid()
                    predictions = predictions.softmax(dim=1).argmax(1)
